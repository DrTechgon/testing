...
def process_files():
    ...
    try:
        ...
        print(f"  [OK] Processed: {successful}", flush=True)
        print(f"  [SKIP] Skipped: {skipped}", flush=True)
        print(f"  [DEL] Deleted: {deleted_count}", flush=True)
...
def chat():
    """Handle chat requests - FAQ first, then LLM"""
    log_step("CHAT", "start")
    ...
    try:
        ...
        if not data or "message" not in data:
            log_step("Validation", "error", "message required")
            return jsonify({
                "success": False,
        ...
        log_step("Message", "info", f"'{user_message[:50]}...'")

        # First, try FAQ matching
        faq_result = find_faq_match(user_message)

        if faq_result.get("matched"):
            log_step("FAQ Match", "success", f"ID: {faq_result.get('faq_id')}")
            return jsonify({
                "success": True,
        ...
        log_step("LLM", "start")
        llm_reply = get_reply(user_message)
        log_step("LLM", "success", f"{len(llm_reply)} chars")
...
if __name__ == "__main__":
    print("\n" + "="*80, flush=True)
    print("START MEDICAL RAG API SERVER", flush=True)
    ...
    print("  POST   /api/chat", flush=True)
